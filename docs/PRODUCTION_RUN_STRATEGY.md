# DGAS Production Run Strategy

**Version**: 1.0  
**Date**: 2025-01-27  
**Purpose**: Comprehensive strategic plan for 24/7 production operation of DGAS with continuous data collection, automated signal generation, and Discord notifications

---

## Executive Summary

This document provides a complete strategy for deploying and operating the Drummond Geometry Analysis System (DGAS) in a 24/7 production environment. The system will:

1. **Continuously collect market data** for 517+ US stocks via EODHD API (24/7)
2. **Generate trading signals** every 15 minutes using Drummond Geometry analysis
...
3. [Data Collection Service Setup](#data-collection-service-setup)
4. [Prediction Scheduler Setup](#prediction-scheduler-setup)
5. [Discord Notification Configuration](#discord-notification-configuration)
...
- `websocket_interval: "5m"` (Aggregate ticks into 5m bars, then to 30m)
...
- Test API connection: `uv run python scripts/test_eodhd_api_direct.py` (if exists)
...
- See [Discord Notification Configuration](#discord-notification-configuration) section
...
- Test Discord API: `uv run python scripts/test_discord_send.py` (if exists)
...
screen -dmS dgas_data_collection bash -c "cd $PROJECT_DIR && source .env && exec uv run dgas data-collection start --config config/production.yaml"

# Start prediction scheduler
echo "Starting prediction scheduler..."
screen -dmS dgas_prediction_scheduler bash -c "cd $PROJECT_DIR && source .env && exec uv run dgas scheduler start --daemon --config config/production.yaml"

# Start dashboard
echo "Starting dashboard..."
screen -dmS dgas_dashboard bash -c "cd $PROJECT_DIR && source .env && exec uv run python run_dashboard.py"
...
psql "$DGAS_DATABASE_URL" -c "SELECT COUNT(*) FROM prediction_runs WHERE run_timestamp > NOW() - INTERVAL '1 hour';"
```
...
The script `scripts/daily_health_check.sh` should check:
...
uv run python scripts/test_discord_send.py  # if exists
...
# 2. Backfill missing data (if backfill script exists)
uv run python scripts/backfill_to_latest.py  # if exists
...
This production run strategy provides a comprehensive plan for deploying and operating DGAS in a 24/7 production environment. Key success factors:
